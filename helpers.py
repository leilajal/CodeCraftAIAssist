...
def get_random_evaluate_question(table_name):
        """
        This function returns a random evaluate question from the database.
        """
        db = DbOps('resources/evaluate_questions.db')

        db.create_table('coding_evaluate_questions')
        db.create_table('coding_evaluate_questions')
        db.create_table('ml_system_design_evaluate_questions')
        
        if table_name == 'coding_evaluate_questions':
            db.populate_db('coding_evaluate_questions', 'resources/sample_coding_questions.txt')
        
        if table_name == 'coding_evaluate_questions':
            db.populate_db('coding_evaluate_questions', 'resources/sample_behavioral_questions.txt')
        
        if table_name == 'ml_system_design_evaluate_questions':
            db.populate_db('ml_system_design_evaluate_questions', 'resources/sample_ML_System_Design_questions.txt')
        
        evaluate_question = db.get_random_element(table_name)
...
def get_transcript_from_youtube_video(video_id, file_path):
    ...
    with open(file_path, 'w', encoding='utf-8') as evaluate:
        evaluate.write(res) 
    return res
...
def calculate_similarity_to_coding_principles(coding_principles, 
                                                  summarized_response, 
                                                  random_story):
    """
    This function calculates the similarity of the summarized response to the coding principles.
    """
    # Define embedding
    embedding = OpenAIEmbeddings(client=None)

    coding_principles_embedding = {}
    # Embedding coding principles
    for key, value in coding_principles.items():
        coding_principles_embedding[key] = embedding.embed_query(value)

    # Embedding summarized text, evaluate response
    ...
    candidate_response_similarity_to_coding_principles = {}

    sum = 0.0
    for key, value in coding_principles_embedding.items():
        candidate_response_similarity_to_coding_principles[key] = abs(inner_vector_product(summarized_response_vector, value) - baseline_weigth)
        sum += candidate_response_similarity_to_coding_principles[key]

    for key, value in candidate_response_similarity_to_coding_principles.items():
        candidate_response_similarity_to_coding_principles[key] = (value/sum)*100

    sorted_candidate_response_similarity_to_coding_principles = sorted(candidate_response_similarity_to_coding_principles.items(), 
                                                                key=lambda x:x[1], 
                                                                reverse=True)
    return sorted_candidate_response_similarity_to_coding_principles
...
def plot_horizontal_bar_chart(data_dict):
    ...
    plt.title('Similarity of the Code Generation Response to the coding Principles')
    plt.xlabel('Percentage')
    plt.ylabel('coding Principles')
...
def generate_pandas_df_from_dict(tuple_list):
    ...
    return pd.DataFrame({"coding Principles":keys, "Percentage":values})
